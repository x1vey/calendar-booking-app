...
function HeroPreview({ block, accent, theme, onCtaClick }: { block: SectionBlock; accent: string; theme?: string; onCtaClick?: () => void }) {
  ...
  return (
    <div className={`lpb-prev-hero ${getThemeClasses(theme)}`} style={{ textAlign: textAlign as any, alignItems: align }}>
      {imageUrl && <img src={imageUrl} alt="" className="lpb-prev-hero-img" style={{ borderRadius: 'var(--ct-radius, 1rem)', border: 'var(--ct-border-width, 0) solid var(--ct-border, transparent)', boxShadow: 'var(--ct-shadow, none)' }} />}
      <h1 className="lpb-prev-h1">{headline || 'Your Headline'}</h1>
      <p className="lpb-prev-sub">{subheadline || 'Your subheadline goes here'}</p>
      <button className="lpb-prev-cta" style={{ background: 'var(--ct-btn-bg, ' + accent + ')', color: 'var(--ct-btn-text, #fff)', border: 'var(--ct-border-width, 0px) solid var(--ct-btn-border, transparent)', borderRadius: 'var(--ct-radius, 0.75rem)' }} onClick={onCtaClick}>{ctaText || 'Book Now'}</button>
    </div>
  );
}

function FeaturesPreview({ block, accent, theme }: { block: SectionBlock; accent: string; theme?: string }) {
  const items = (block.props.items || []) as { icon: string; title: string; description: string }[];
  return (
    <div className={`lpb-prev-section ${getThemeClasses(theme)}`}>
...
function VideoPreview({ block, theme }: { block: SectionBlock; theme?: string }) {
...
function TestimonialsPreview({ block, accent, theme }: { block: SectionBlock; accent: string; theme?: string }) {
...
function ExpectationsPreview({ block, accent, theme }: { block: SectionBlock; accent: string; theme?: string }) {
  ...
  return (
    <div className={`lpb-prev-cta-section ${getThemeClasses(theme)}`} style={{ background: accent + '08' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <button className="lpb-prev-cta" style={{ background: 'var(--ct-btn-bg, ' + accent + ')', color: 'var(--ct-btn-text, #fff)', border: 'var(--ct-border-width, 0px) solid var(--ct-btn-border, transparent)', borderRadius: 'var(--ct-radius, 0.75rem)' }} onClick={onCtaClick}>{block.props.ctaText || 'Book Now'}</button>
      </div>
    </div>
  );
}

function FaqPreview({ block, accent, theme }: { block: SectionBlock; accent: string; theme?: string }) {
  ...
  return (
    <div className={`lpb-prev-section ${getThemeClasses(theme)}`}>
      ...
      <div className="lpb-prev-faq-list">
        {items.map((item, i) => (
          <div key={i} className="lpb-prev-faq-item" style={{ borderRadius: 'var(--ct-radius, 0.75rem)', border: 'var(--ct-border-width, 1px) solid var(--ct-border, rgba(0,0,0,0.06))' }} onClick={() => setOpenIdx(openIdx === i ? null : i)}>
...
function StatsPreview({ block, accent, theme }: { block: SectionBlock; accent: string; theme?: string }) {
...
function LogoStripPreview({ block, theme }: { block: SectionBlock; theme?: string }) {
