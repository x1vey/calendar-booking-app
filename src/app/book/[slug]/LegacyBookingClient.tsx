...
function BookingPageInner({ slug }: { slug: string }) {
  ...
  const handleBooking = async (e?: React.FormEvent, provider?: string) => {
    ...
    try {
      ...
      if (data.requiresPayment && provider) {
        // Proceed to payment gateway
        const intentRes = await fetch('/api/payments/create-intent', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ calendarId: calendar!.id, provider, bookerName: formData.name, bookerEmail: formData.email, bookingId: data.booking.id }),
        });
          ...
          script.onload = () => {
            const options = {
              ...
              handler: function (response: any) {
                window.location.href = `/book/${slug}/confirmed?id=${data.booking.id}&payment_id=${response.razorpay_payment_id}`;
              },
              prefill: { name: formData.name, email: formData.email },
              theme: { color: calendar!.theme_bg_color || '#3399cc' }
            };
  ...
  if (isEmbed) {
    return (
      <div style={{ ...themeVars, backgroundColor: 'transparent', color: 'var(--theme-text)' }} className="selection:bg-blue-600 selection:text-white font-sans">
        <div id="booking-grid" className="w-full">
          <div className={getCalendarClasses()} style={{ backgroundColor: 'var(--cal-bg)', color: 'var(--cal-text)' }}>
            ...
            {step === 'form' && (
              <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full">
                ...
                <form onSubmit={e => handleBooking(e, 'none')} className="space-y-5">
                  ...
                  {calendar.require_payment ? (
                    <div className="pt-4 space-y-3">
                      <p className="text-sm font-bold text-center mb-2" style={{ color: 'var(--theme-text)' }}>Total: {calendar.price} {calendar.currency}</p>
                      {calendar.stripe_enabled && (
                        <Button type="button" onClick={() => handleBooking(undefined, 'stripe')} disabled={bookingLoading} className="w-full btn-pill h-14 text-sm font-black shadow-xl" style={{ backgroundColor: '#6366f1', color: '#fff' }}>
                          {bookingLoading ? 'Processing...' : 'Pay with Stripe (Card)'}
                        </Button>
                      )}
                      {calendar.paypal_enabled && (
                        <Button type="button" onClick={() => handleBooking(undefined, 'paypal')} disabled={bookingLoading} className="w-full btn-pill h-14 text-sm font-black shadow-xl" style={{ backgroundColor: '#0070ba', color: '#fff' }}>
                          {bookingLoading ? 'Processing...' : 'Pay with PayPal'}
                        </Button>
                      )}
                      {calendar.razorpay_enabled && (
                        <Button type="button" onClick={() => handleBooking(undefined, 'razorpay')} disabled={bookingLoading} className="w-full btn-pill h-14 text-sm font-black shadow-xl" style={{ backgroundColor: '#3399cc', color: '#fff' }}>
                          {bookingLoading ? 'Processing...' : 'Pay with Razorpay'}
                        </Button>
                      )}
                      {!calendar.stripe_enabled && !calendar.paypal_enabled && !calendar.razorpay_enabled && (
                        <p className="text-xs text-rose-500 font-bold text-center">Payment is required but no gateways are configured.</p>
                      )}
                    </div>
                  ) : (
                    <div className="pt-2">
  ...
  return (
    <div style={{ ...themeVars, backgroundColor: 'var(--theme-bg)', color: 'var(--theme-text)' }} className="min-h-screen flex flex-col items-center pb-20 landing-body selection:bg-blue-600 selection:text-white">
      ...
      {pageLayout ? (
        ...
        {/* ── BOOKING CALENDAR ── */}
        <div id="booking-grid" className="scroll-mt-32 max-w-4xl mx-auto w-full z-10 relative">
          <div className={getCalendarClasses()} style={{ backgroundColor: 'var(--cal-bg)', color: 'var(--cal-text)' }}>
        ...
        {/* ── GOOGLE REVIEWS (Legacy) ── */}
        {!pageLayout && calendar.google_place_id && (
          <div className="w-full max-w-4xl mx-auto bg-white rounded-[3rem] p-8 md:p-12 shadow-2xl ring-1 ring-slate-200/50 text-slate-900 mt-24">
        ...
        {/* ── TESTIMONIAL VIDEO (Legacy) ── */}
        {!pageLayout && testimonialList.length > 0 && (
          <div className="w-full max-w-4xl mx-auto text-center mt-24" style={{ paddingBottom: '2rem' }}>
...
export default function LegacyBookingClient({ slug }: { slug: string }) {
  return (
